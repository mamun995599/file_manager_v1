...
$proxy_url = 'file_proxy.php?file=' . urlencode(trim(FM_PATH . '/' . $file));
...
?>
    <div class="row">
        <div class="col-12">
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                <?php endif; ?>
                <a class="fw-bold btn btn-outline-primary" href="<?php echo fm_enc($proxy_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                <?php
                // ZIP actions
    ...
    $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
        if ($ext === 'pdf') {
    $proxy_pdf = '../pdf_proxy.php?file=' . urlencode(trim(FM_PATH . '/' . $file));
    $viewerUrl = 'web/viewer.html?file=' . urlencode($proxy_pdf);
    echo '<iframe src="' . $viewerUrl . '" frameborder="0" style="width:100%;min-height:460px"></iframe>';
} else {
    if ($online_viewer === 'google') {
        echo '<iframe src="https://docs.google.com/viewer?embedded=true&hl=en&url=' . fm_enc($proxy_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
    } elseif ($online_viewer === 'microsoft') {
        echo '<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=' . fm_enc($proxy_url) . '" frameborder="no" style="width:100%;min-height:460px"></iframe>';
    }
} }elseif ($is_zip) {
                    // ZIP content
                    if ($filenames !== false) {
                    ...
                    if (in_array($ext, array('gif', 'jpg', 'jpeg', 'png', 'bmp', 'ico', 'svg', 'webp', 'avif'))) {
                        echo '<p><input type="checkbox" id="preview-img-zoomCheck"><label for="preview-img-zoomCheck"><img src="' . fm_enc($proxy_url) . '" alt="image" class="preview-img"></label></p>';
                    }
                } elseif ($is_audio) {
                    // Audio content
                    echo '<p><audio src="' . fm_enc($proxy_url) . '" controls preload="metadata"></audio></p>';
                } elseif ($is_video) {
                    // Video content
                    echo '<div class="preview-video"><video src="' . fm_enc($proxy_url) . '" width="640" height="360" controls preload="metadata"></video></div>';
                } elseif ($is_text) {
            ...
            ?>
                <tr>
                    ...
                    <td class="inline-actions">
                        ...
                        <?php endif; ?>
<a title="<?php echo lng('DirectLink') ?>" href="file_proxy.php?file=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
